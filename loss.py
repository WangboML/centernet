...
def _neg_loss(preds, gt):
    ...
    neg_weights = torch.pow(1 - gt[neg_inds], 4)# 先求1-Pcij的4次方
...
def _sigmoid(x):
    x = torch.clamp(x.sigmoid_(), min=1e-4, max=1 - 1e-4) #将输入input张量每个元素的夹紧到区间 [min,max],并返回结果到一个新张量
    return x
...
class AELoss(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        return loss.unsqueeze(0), (focal_loss / len(tl_heats)).unsqueeze(0), (pull_loss / len(tl_heats)).unsqueeze(0), (
                    push_loss / len(tl_heats)).unsqueeze(0), (regr_loss / len(tl_heats)).unsqueeze(0)

losses = AELoss(pull_weight=1e-1, push_weight=1e-1, focal_loss=_neg_loss)